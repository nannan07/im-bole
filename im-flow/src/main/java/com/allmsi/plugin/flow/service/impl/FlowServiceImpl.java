...
import com.allmsi.plugin.flow.dao.FlowAuthMapper;
import com.allmsi.plugin.flow.dao.FlowInstanceMapper;
import com.allmsi.plugin.flow.dao.FlowMapper;
import com.allmsi.plugin.flow.dao.FlowNodeMapper;
import com.allmsi.plugin.flow.dao.FlowRouteMapper;
import com.allmsi.plugin.flow.model.Flow;
import com.allmsi.plugin.flow.model.FlowDealType;
import com.allmsi.plugin.flow.model.external.FlowUserModel;
import com.allmsi.plugin.flow.model.ivo.FlowIVo;
import com.allmsi.plugin.flow.model.ovo.Flow4ListOVo;
import com.allmsi.plugin.flow.model.ovo.FlowOVo;
import com.allmsi.plugin.flow.service.FlowService;
...
public class FlowServiceImpl implements FlowService {
	...
	private FlowOVo selectByPrimaryKey(String id) {
		...
		if (flow != null) {
			FlowOVo flowOVo = new FlowOVo(flow);
			if (StrUtil.notEmpty(flow.getcUserId())) {
				Map<String, FlowUserModel> map = flowUserService.getFlowUserSingle(flow.getcUserId());
