...
import com.allmsi.plugin.flow.service.FlowNodeDealService;
...
public class FlowNodeDealServiceImpl implements FlowNodeDealService {
	...
	public FlowNodeDealOVo selectFlowNodeDealInfo(String id) {
		...
		if (fnd == null) {
			...
			if(StrUtil.notEmpty(fnd.getcUserId())){
				Map<String, FlowUserModel> flowUserMap = flowUserService.getFlowUserSingle(fnd.getcUserId());
				fndVo.setUser(flowUserMap.get(fnd.getcUserId()));
			}
			String nodeDealName = flowUserService.getFlowDealSingle(fnd.getNodeDealId(),fnd.getNodeDealType());
